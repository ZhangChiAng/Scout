...
及接收者类型与 ID；个性化发送必须使用 `chat_id` 群目标。按需正文以原列表保存
...
防止 timer 与人工发送重叠；listener 用独立文件锁防止同时启动两个后台发送线程。
...
当前这一轮：暂停并停用 timer，确认 sender 空闲，增量初始化数据库并重启
listener；仅执行 `--send --issue-date 2026-09-02` 处理剩余 8 条，已送达正文
不重发。完成后保留 listener，等待 owner 观察反馈，不自动推进下一日期或恢复 timer。
